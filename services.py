...
from aiogram.types import ChatPermissions, InputFile, BufferedInputFile
...
def _until_date(duration_key: str) -> datetime | None:
    """Возвращает datetime окончания или None (навсегда)."""
    seconds = DURATIONS.get(duration_key, 0)
    if seconds == 0:
        return None  # навсегда / без срока
    return datetime.now(timezone.utc) + timedelta(seconds=seconds)


def _expires_ts(duration_key: str) -> int:
    """Unix-время окончания (0 = навсегда)."""
    seconds = DURATIONS.get(duration_key, 0)
    if seconds == 0:
        return 0
...
async def apply_mute(
    ...
    """Мутит пользователя. Возвращает текст результата."""
...
async def apply_prefix(
    ...
    """Устанавливает кастомный титул администратора (работает только для админов)."""
    try:
        # Сначала повышаем до админа, если ещё не админ
...
async def apply_avatar(
    ...
    """Меняет фото группы."""
    try:
        # Скачиваем файл и загружаем как аватарку
...
async def apply_lock(
    ...
    """Блокирует чат — включает slow mode (максимальный) или снимает право отправлять сообщения."""
    try:
        # Ограничиваем всех участников
    ...
    try:
        if svc == "mute" and target_id:
            ...
            try:
                ...
                pass
            logger.info(f"Авто-снятие префикса: user {target_id}")
